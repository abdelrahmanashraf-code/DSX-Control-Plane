...
const NAV_ITEMS = [
  ["الرئيسية", "لوحة التحكم", true],
  ["التجارب", "Trials", false],
  ["العملاء", "Tenants", false],
  ["السيرفرات", "Nodes", false],
  ["العمليات", "Provisioning", false],
  ["النسخ", "Backups & Restore", false],
  ["التنبيهات", "Alerts", false],
  ["السجل", "Audit Log", false],
] as const;
    ...
    <main className="shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list" aria-label="التنقل الرئيسي">
          {NAV_ITEMS.map(([arabic, english, active]) => (
            <div className={active ? "nav-item active" : "nav-item disabled"} key={english}>
              <span>{arabic}</span>
              <small>{english}</small>
            </div>
          ))}
      ...
      <section className="content">
        ...
        <section className="phase-banner">
          <div>
            ...
            <h2>Create Trial من الواجهة</h2>
            <p>الخطوة التالية: شاشة Trials + إنشاء تجربة 3 أيام وربطها تلقائيًا بالـProvisioning والـtrial pool.</p>
          </div>
          <span>Phase 5</span>
        </section>
      </section>
